...
const CurrencyTrendsChart: React.FC<CurrencyTrendsChartProps> = ({ trends }) => {
  const [selectedCurrency, setSelectedCurrency] = useState<string>(trends[0]?.currency || 'EUR');
  
  const selectedTrend = trends.find(trend => trend.currency === selectedCurrency);
  ...
  const lastValue = selectedTrend.data[selectedTrend.data.length - 1]?.value || 0;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="flex items-center mb-4">
        ...
        <span className={`ml-2 text-sm font-medium ${trendDirection === 'up' ? 'text-green-500' : 'text-red-500'}`}>
          {trendDirection === 'up' ? '▲' : '▼'} {Math.abs(percentChange).toFixed(2)}%
        </span>
        ...
        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
          <defs>
            <linearGradient id="gradient" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor={trendDirection === 'up' ? 'rgb(34, 197, 94, 0.2)' : 'rgb(239, 68, 68, 0.2)'} />
